...
fastq/%.1.fastq.gz fastq/%.2.fastq.gz : unprocessed_bam/%.fixmate.bam
	...
	$(call SAM_TO_FASTQ_MEM,8G) QUIET=true I=$< FASTQ=$${TEMP}_1 SECOND_END_FASTQ=$${TEMP}_2 &> $(LOGDIR)/$(@F).log
else
fastq/%.1.fastq.gz fastq/%.2.fastq.gz : %.bam
	$(call INIT_MEM,10G,40G) $(BAM2FASTQ) -o fastq/$*#.fastq $< &> $(LOGDIR)/$(@F).bam2fastq.log && gzip < fastq/$*_1.fastq > fastq/$*.1.fastq.gz && gzip < fastq/$*_2.fastq > fastq/$*.2.fastq.gz
endif

fastq/%.readtrim.1.fastq.gz fastq/%.readtrim.2.fastq.gz : %.bam %.read_len
	$(call INIT_MEM,10G,12G) \
	...
	$(SAM_TO_FASTQ) I=$< FASTQ=$${TEMP}_1 SECOND_END_FASTQ=$${TEMP}_2 READ1_MAX_BASES_TO_WRITE=$$MAX_LENGTH READ2_MAX_BASES_TO_WRITE=$$MAX_LENGTH &> $(LOGDIR)/$(@F).log

%.read_len : %.bam
	$(call INIT_MEM,4G,5G) $(SAMTOOLS) view $< | awk '{ print length($$10) }' | sort -n | uniq -c | sort -rn | sed 's/^ \+//' | awk ' > $@
