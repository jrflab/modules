...
soapfuse/%.timestamp : soapfuse/sample_lists/%.txt
	$(call LSCRIPT_NAMED_PARALLEL_MEM,$*_soapfuse,4,3G,5G,"$(SOAPFUSE) -c $(SOAPFUSE_CONFIG) -fd $(@D) -l $< -o $(@D)/$* && touch $@ && $(RM) -rf soapfuse/$*/alignWG")
...
soapfuse/tables/%.sfuse.txt : soapfuse/%.timestamp
	$(INIT) cp soapfuse/$*/final_fusion_genes/$*/$*.final.Fusion.specific.for.genes $@

soapfuse/tables/%.sfuse_isoforms.txt : soapfuse/%.timestamp
	$(INIT) cp soapfuse/$*/final_fusion_genes/$*/$*.final.Fusion.specific.for.trans $@
