...
vcf/ann/mutass/%.mutass.vcf : vcf/ft/%.vcf
...
vcf/ann/nsfp/%.nsfp.vcf : vcf/ft/%.vcf vcf/ft/%.vcf.idx
...
vcf/ann/gene_ann/%.gene_ann.vcf : vcf/ft/%.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(ADD_GENE_LIST_ANNOTATION) --genome $(REF) --geneBed $(HAPLOTYPE_INSUF_BED)$(,)$(CANCER_GENE_CENSUS_BED)$(,)$(KANDOTH_BED)$(,)$(LAWRENCE_BED) --name hap_insuf$(,)cancer_gene_census$(,)kandoth$(,)lawrence --outFile $@ $<")
...
vcf/ann/cn_reg/%.cn_reg.vcf : vcf/ft/%.vcf
	$(call LSCRIPT_MEM,8G,12G,"$(ADD_GENE_LIST_ANNOTATION) --genome $(REF) --geneBed $(subst $(space),$(,),$(strip $(CN_ENDOMETRIAL_BED)) $(strip $(CN_BREAST_BED))) --name $(subst $(space),$(,),$(foreach set,$(strip $(CN_ENDOMETRIAL_SUBTYPES)),endometrial_$(set)) $(foreach set,$(strip $(CN_BREAST_SUBTYPES)),breast_$(set))) --outFile $@ $<")
