...
vcf/%.dbsnp.vcf : vcf/%.vcf
	$(call CHECK_VCF,$(call LSCRIPT_CHECK_MEM,20G,23G,"$(call SNP_SIFT_MEM,12G) annotate \
		$(SNP_SIFT_OPTS) $(DBSNP) $< > $@ && grep -q '^#CHROM' $@"))

vcf/%.hotspot_ann.vcf : vcf/%.vcf
	$(call CHECK_VCF,$(call LSCRIPT_CHECK_MEM,20G,23G,"$(call SNP_SIFT_MEM,12G) annotate $(SNP_SIFT_OPTS) \
...
vcf/%.eff.vcf : vcf/%.vcf
	$(call CHECK_VCF,$(call LSCRIPT_CHECK_MEM,18G,20G,"$(call SNP_EFF_MEM,11G) ann $(SNP_EFF_OPTS) $(SNP_EFF_GENOME) -s $(@D)/$*.eff_summary.html $< > $@"))
