...
vcf/%.fp_ft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(call GATK_MEM,8G) -T VariantFiltration -R $(REF_FASTA) -V $< -o $@.tmp --maskName 'FuentesFalsePositive' --mask $(FALSE_POSITIVE_BED) && $(call VERIFY_VCF,$@.tmp,$@)")

DGD_BED = $(HOME)/share/reference/dgd.include_cosmic.hg19.bed
vcf/%.dgd_ft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(call GATK_MEM,8G) -T VariantFiltration -R $(REF_FASTA) -V $< -o $@.tmp --maskName 'DuplicateGenesDB' --mask $(DGD_BED) && $(call VERIFY_VCF,$@.tmp,$@)")

ENCODE_BED = $(HOME)/share/reference/wgEncodeDacMapabilityConsensusExcludable.include_cosmic.bed
vcf/%.encode_ft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(call GATK_MEM,8G) -T VariantFiltration -R $(REF_FASTA) -V $< -o $@.tmp --maskName 'encode' --mask $(ENCODE_BED) && $(call VERIFY_VCF,$@.tmp,$@)")

%.het_ft.vcf : %.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(call GATK_MEM,8G) -T VariantFiltration -R $(REF_FASTA) -V $< -o $@.tmp \
...
vcf/%.vaf_ft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,2G,5G,"$(call SNP_SIFT_MEM,2G) filter $(SNP_SIFT_OPTS) '(exists GEN[*].VAF) & (GEN[0].VAF > 0.05) & (GEN[1].VAF < 0.05)' < $< > $@.tmp && $(call VERIFY_VCF,$@.tmp,$@)")

# target region filter
INTERVAL_FILTER_VCF = python modules/vcf_tools/interval_filter_vcf.py
%.target_ft.vcf : %.vcf
	$(call LSCRIPT_CHECK_MEM,4G,6G,"$(INTERVAL_FILTER_VCF) $(TARGETS_FILE) $< > $@.tmp && $(call VERIFY_VCF,$@.tmp,$@)")
...
%.target_dp_ft.vcf : %.vcf
	$(call LSCRIPT_CHECK_MEM,4G,6G,"$(INTERVAL_DEPTH_FILTER_VCF) --depth_threshold $(INTERVAL_DEPTH_THRESHOLD) \
...
vcf/%.vdp_ft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,2G,5G,"cat $< | $(call SNP_SIFT_MEM,2G) filter $(SNP_SIFT_OPTS) '(exists GEN[*].AD) & (GEN[*].AD > $(DEPTH_FILTER))' > $@.tmp && $(call VERIFY_VCF,$@.tmp,$@)")
...
vcf/$1_$2.%.som_ad_ft.vcf : vcf/$1_$2.%.vcf
	$$(call LSCRIPT_CHECK_MEM,4G,7G,"$$(MUTECT2_SOMATIC_AD_FILTER_VCF) --tumor $1 --normal $2 $$< > $$@.tmp && $$(call VERIFY_VCF,$$@.tmp,$$@)")
...
ifdef NORMAL_VCF
vcf/%.nft.vcf : vcf/%.vcf
	$(call LSCRIPT_CHECK_MEM,8G,12G,"$(call GATK_MEM,8G) -T VariantFiltration -R $(REF_FASTA) -V $< -o $@.tmp --maskName 'normal' --mask $(NORMAL_VCF) && $(call VERIFY_VCF,$@.tmp,$@)")
