...
for(sub.run in c('nonsyn', 'syn_nonsyn')) {
    ...
    SubGroup <- function(sub.num) {
        ...
        if(stop.sub.0 == TRUE) { QuietStop('- exiting after: sub.num = 0 //') }
        ...
        if(run.fishers.plots == TRUE & !is.na(sub.group$b)) {
