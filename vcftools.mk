...
%.ss_dp_ft.vcf : %.vcf
	$(call LSCRIPT_MEM,2G,5G,"$(call SNP_SIFT_MEM,2G) filter -p -a DP -i Depth -r PASS -f $< '(exists GEN[*].DP) & (GEN[ALL].DP >= $(DEPTH_FILTER))' > $@")
...
%.chasm.vcf : %.vcf
	$(call LSCRIPT_MEM,8G,12G,"$(CHASM) --genome $(REF) --chasmDir $(CHASM_DIR) --python $(CHASM_PYTHON) --outFile $@ $<")
