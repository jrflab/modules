...
bwa/bam/%.bwa.bam : bwa/sai/%.1.sai bwa/sai/%.2.sai fastq/%.1.fastq.gz fastq/%.2.fastq.gz
	$(call INIT_MEM,4G,10G) \
	LBID=`echo "$*" | sed 's/_[0-9]\+//'`; \
	$(BWA) sampe -P -r "@RG\tID:$*\tLB:$${LBID}\tPL:${SEQ_PLATFORM}\tSM:$${LBID}" $(REF_FASTA) $^ 2> $(LOGDIR)/$@.log | $(SAMTOOLS) view -bhS - > $@
endif
else
bwa/sai/%.1.sai : %.bam
...
bwa/bam/%.bwa.bam : bwa/sai/%.1.sai bwa/sai/%.2.sai %.bam
	$(call INIT_MEM,4G,8G) \
	LBID=`echo "$*" | sed 's/_[0-9]\+//'`; \
	$(BWA) sampe -P -r "@RG\tID:$*\tLB:$${LBID}\tPL:${SEQ_PLATFORM}\tSM:$${LBID}" $(REF_FASTA) $(word 1,$^) $(word 2, $^) $(word 3,$^) $(word 3,$^) 2> $(LOGDIR)/$@.log | $(SAMTOOLS) view -bhS - > $@ && $(RM) $<
endif
