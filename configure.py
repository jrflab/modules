...
if not os.path.isfile(args.samples_file):
    print("sample yaml file does not exist: {}".format(args.samples_file))
    print("skipping")
else:
    sf = open(args.samples_file, 'r')
    samples = yaml.load(sf)
    sf.close()
...
if not os.path.isfile(args.sample_fastq_file):
