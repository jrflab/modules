...
'plot_log2_ratio' <- function(x)
{
	...
	axis(1, at = c(y$start, y$end[nrow(y)]), labels = rep(" ", nrow(y)+1), cex.axis = 0.85, las = 1, lwd = 0, tck = .5)
...
if (as.numeric(opt$option) == 1) {
	...
	pdf(file = paste0("cnvkit/plots/segmented/", opt$sample_name, ".pdf"), width = 7, height = 4.25)
