...
metrics/%.gc_bias_metrics : bam/%.bam
	$(call RUN,-s 12G -m 13G,"$(COLLECT_GC_METRICS) I=$< O=$@ CHART_OUTPUT=$(addsuffix .pdf,$@) REFERENCE_SEQUENCE=$(REF_FASTA)")

metrics/%.flagstats : bam/%.bam
	$(call RUN,-s 2G -m 3G,"$(SAMTOOLS) flagstat $< > $@")
	
bam/%.markdup.bam metrics/%.dup_metrics : bam/%.bam
	$(call RUN,-s 18G -m 19G,"$(MARK_DUP) I=$< O=bam/$*.markdup.bam METRICS_FILE=metrics/$*.dup_metrics")
