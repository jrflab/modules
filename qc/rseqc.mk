...
rseqc/infer/%.infer : bam/%.bam bam/%.bam.bai
	$(call LSCRIPT_MEM,7G,"$(INFER_EXPT) -i $< -r $(REF_HOUSEKEEPING_GENE_BED) > $@")

rseqc/gene_body_cov/%.geneBodyCoverage.txt : bam/%.bam
	$(call LSCRIPT_MEM,7G,"$(GENEBODY_COV) -i $< -r $(REF_HOUSEKEEPING_GENE_BED) -o rseqc/gene_body_cov/$*")

rseqc/inner_dist/%.innerDistance.txt : bam/%.bam
	$(call LSCRIPT_MEM,7G,"$(INNER_DIST) -i $< -r $(REF_HOUSEKEEPING_GENE_BED) -o rseqc/inner_dist/$*")
