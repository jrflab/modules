...
mmTranscribed <- motifMatrix(subset(allVr, allVr$transcribed), normalize = T)
mmUntranscribed <- motifMatrix(subset(allVr, !allVr$transcribed), normalize = T)
if (!is.null(opt$targetBed)) {
...
rownames(Y) <- paste(sub('>', '', as.character(Y$Substitution.Type)),
                              ' ', subseq(as.character(Y$Trinucleotide), 1, 1), '.',
                              subseq(as.character(Y$Trinucleotide), 3, 3), sep = '')
...
plotMutBarplot <- function(samp, mm) {
    ...
    mdf %>% filter(sample == samp) %>%
        ...
        geom_bar(stat = 'identity') + facet_grid(~ alteration, scales = 'free_x') +
...
plotMutBarplotStranded <- function(samp, mmTranscribed, mmUntranscribed) {
    ...
    mdf %>% filter(sample == samp) %>%
        ...
        geom_bar(stat = 'identity', position = 'dodge') + facet_grid(~ alteration, scales = 'free_x') +
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        scale_fill_manual(name = "", values = c('blue', 'red'), labels = c("Transcribed strand", 'Untranscribed strand')) + 
        xlab("") + ylab("") + ggtitle(tit)
}
...
nmfCoefs <- solveNMF(as.matrix(alexandrovM), as.matrix(mmNorm))
