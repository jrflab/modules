...
plotMutBarplot <- function(X, tit) {
    cols <- c("C>A" = "lightblue", "C>G" = "black", "C>T" = "red", "T>A" = "grey", "T>C" = "lightgreen",  "T>G" = "pink")
    plot1 <- ggplot(subset(X, grepl('^C', Substitution.Type)), aes(x=factor(Trinucleotide), y = value, fill = Substitution.Type)) + geom_bar(stat = 'identity') + facet_wrap(~ Substitution.Type) + ylim(0, max(X$value)) + xlab("") + ylab("") + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = 'none') + scale_fill_manual(values = cols)
    plot2 <- ggplot(subset(X, grepl('^T', Substitution.Type)), aes(x=factor(Trinucleotide), y = value, fill = Substitution.Type)) + geom_bar(stat = 'identity') + facet_wrap(~ Substitution.Type) + ylim(0, max(X$value)) + xlab("") + ylab("") + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = 'none') + scale_fill_manual(values = cols)
    grid.arrange(plot1, plot2, nrow = 2, main = tit)
}
