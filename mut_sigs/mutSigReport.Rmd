...
for (vcfFile in vcfFiles) {
    ...
    if (nrow(vcf) > 0) {
        ...
        ol <- findOverlaps(rowData(vcf), txByGenes)
        ...
        info(vcf)$transcribed[rowData(vcf)$MUT %in% c("G>A", "G>C", "G>T", "A>C", "A>G", "A>T") & grepl('\\+', info(vcf)$strand)] <- T
        info(vcf)$transcribed[rowData(vcf)$MUT %in% c("C>A", "C>G", "C>T", "T>A", "T>C", "T>G") & grepl('-', info(vcf)$strand)] <- T
...
for (s in names(vcfs)) {
    ...
    main <- paste(s, " (n=", length(rowData(vcf)), ")", sep = '')
    pie(table(rowData(vcf)$MUT), col = cols, main = main)
    if (sum(info(vcf)$transcribed) > 0 && sum(!info(vcf)$transcribed) > 0) {
        main <- paste('transcribed', s, " (n=", sum(info(vcf)$transcribed), ")", sep = '')
        pie(table(rowData(subset(vcf, info(vcf)$transcribed))$MUT), col = cols, main = main)
        main <- paste('untranscribed', s, " (n=", sum(!info(vcf)$transcribed), ")", sep = '')
        pie(table(rowData(subset(vcf, !info(vcf)$transcribed))$MUT), col = cols, main = paste('untranscribed', main))
    }
}
```
...
for (s in names(vcfs)) {
    vcf <- vcfs[[s]]
    mutTabs[[s]] <- createMutationTable(vcf)
    if (sum(info(vcf)$transcribed) > 0 && sum(!info(vcf)$transcribed) > 0) {
        s1 <- paste('transcribed', s)
        mutTabs[[s1]] <- createMutationTable(subset(vcf, info(vcf)$transcribed))
        s2 <- paste('untranscribed', s)
        mutTabs[[s2]] <- createMutationTable(subset(vcf, !info(vcf)$transcribed))
    }
}
```
...
createMutationTable <- function(vcf) {
    svcf <- split(rowData(vcf), rowData(vcf)$MUT)
    tabs <- list()
