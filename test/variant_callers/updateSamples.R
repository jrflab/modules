...
# 	alpha_hat = cancercellFraction(f_hat, n, qt, q2, ifelse((alpha[index]-.25)<=0, alpha[index], alpha[index]-.25), e=0.01)
	cancer_cell_fraction = cbind(cancer_cell_fraction, rep(NA, nrow(vars))) #cancer_cell_fraction = cbind(cancer_cell_fraction, alpha_hat[,"cancer_cell_frac"])
	ccf_95CI_low = cbind(ccf_95CI_low, rep(NA, nrow(vars))) #ccf_95CI_low = cbind(ccf_95CI_low, alpha_hat[,"ccf_95CI_low"])
	ccf_95CI_high = cbind(ccf_95CI_high, rep(NA, nrow(vars))) #ccf_95CI_high = cbind(ccf_95CI_high, alpha_hat[,"ccf_95CI_high"])
	pr_somatic_clonal = cbind(pr_somatic_clonal, rep(NA, nrow(vars))) #pr_somatic_clonal = cbind(pr_somatic_clonal, alpha_hat[,"Pr_somatic_clonal"])
	ll = cbind(ll, rep(NA, nrow(vars))) #ll = cbind(ll, alpha_hat[,"LL"])
	sq = cbind(sq, rep(NA, nrow(vars))) #sq = cbind(sq, alpha_hat[,"sq"])
	#clonal_estimate = rep("Subclonal", nrow(vars))
	#clonal_estimate[cancer_cell_fraction>.75 | pr_somatic_clonal>.5 | ccf_95CI_low>.9] = "Clonal"
	clonal_status = cbind(clonal_status, rep(NA, nrow(vars))) #clonal_status = cbind(clonal_status, clonal_estimate)
}
