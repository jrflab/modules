...
write.table(desc, file=paste0("medicc/medicc/", opt$sample_set, "/desc.txt"), sep=" ", col.names=FALSE, row.names=FALSE, quote=FALSE, append=FALSE)
for (i in unique(tmp[,"Chromosome"])) {
	cat(">diploid\n", file=paste0("medicc/medicc/", opt$sample_set, "/major_chr", i, ".fasta"), append=FALSE)
	cat(paste0(rep(1, sum(tmp[,"Chromosome"]==i)), collapse=""), "\n", file=paste0("medicc/medicc/", opt$sample_set, "/major_chr", i, ".fasta"), append=TRUE)
	for (j in 1:ncol(q2)) {
	...
	cat(">diploid\n", file=paste0("medicc/medicc/", opt$sample_set, "/minor_chr", i, ".fasta"), append=FALSE)
	cat(paste0(rep(1, sum(tmp[,"Chromosome"]==i)), collapse=""), "\n", file=paste0("medicc/medicc/", opt$sample_set, "/minor_chr", i, ".fasta"), append=TRUE)
	for (j in 1:ncol(q1)) {
