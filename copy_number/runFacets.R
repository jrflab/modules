...
optList <- list(make_option("--snp_nbhd", default = 250, type = 'integer', help = "window size"),
...
'formatSegmentOutput' <- function(out, sampID) {
	...
	for(i in 1:nrow(out$out)) {
		lims = range(out$jointseg$maploc[(out$jointseg$chrom==out$out$chr[i] & out$jointseg$seg==out$out$seg[i])],na.rm = TRUE)
