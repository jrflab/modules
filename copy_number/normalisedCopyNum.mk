...
norm_copynum/doc/%.doc : bam/%.bam norm_copynum/targets.bed
	$(call RUN,-s 9G -m 15G,"$(call GATK_MEM,7G) -T DepthOfCoverage -R $(REF_FASTA) -I $< -L $(<<) -o $@")

norm_copynum/seg.txt : norm_copynum/targets.nuc.bed $(foreach sample,$(SAMPLES),norm_copynum/doc/$(sample).doc)
	$(call RUN,-s 8G -m 10G,"$(NORMALISE_COPYNUM) $(NORMALISE_COPYNUM_OPTS) --sampleSetsFile $(SAMPLE_SET_FILE) --nucFile $< --centromereFile $(CENTROMERE_TABLE2) --outDir $(@D) $(addsuffix .sample_interval_summary,$(filter %.doc,$^))")
