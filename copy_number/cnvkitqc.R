...
plot(0, 0, type="n", axes = FALSE, frame.plot = FALSE, main = "", xlab = "", ylab = "", xlim=c(0, max(x, y)+.5), ylim=c(0,1.2))
polygon(x=c(dx$x, rev(dx$x)), y=c(dx$y, rep(0, length(dx$y))), border="salmon", col=transparentRgb("salmon", 155), lwd=2)
polygon(x=c(dy$x, rev(dy$x)), y=c(dy$y, rep(0, length(dy$y))), border="steelblue", col=transparentRgb("steelblue", 155), lwd=2)
legend("topright", col=c("salmon", "steelblue"), pch=15, legend=c("Tumor", "Normal"), box.lwd=-1)
